...
const Navbar = () => {
  ...
  return (
    <div className="rounded-div flex items-center justify-between h-20 font-bold">
      <Link to="/">
        <h1 className="text-2xl">CoinWorld</h1>
      </Link>
      ...
      {user?.email ? (
        <div>
          ...
          <button onClick={handleSignOut} className="p-2 hover:text-accent">
            Sign Out
          </button>
        </div>
      ) : (
        ...
        {!user ? (
          <div className="flex flex-col w-full p-4">
            ...
            <Link to="signup">
              <button
                onClick={handleNav}
                className="w-full shadow-xl rounded-2xl my-2 p-3 bg-button  "
              >
                Sign Up
              </button>
            </Link>
